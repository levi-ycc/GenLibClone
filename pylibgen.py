        ...
        if elec.isnumeric():
            ...
            for choice in range(page*25-25, page*25):
                if choice < len(books) and choice >= 0:  # Selection
                    ...
                    downloaded = [f[:10] for f in os.listdir(DOWNLOAD_PATH)]
...
class DownloadBook():
    ...
    def save_book(download_link, file_name):
        ...
        try:
            ...
            path = '{}/{}'.format(DOWNLOAD_PATH, file_name.split('  ')[0]+'.'+file_name.split('.')[-1])
            request.urlretrieve(download_link, filename=path)
...
if __name__ == '__main__':
    ...
    column.add_argument('-s', '--series', action='store_true',
                        help='get books from the series')
